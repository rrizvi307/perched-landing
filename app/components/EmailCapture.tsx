...
export function EmailCapture() {
  ...
  if (status === "success") {
    return (
      <div style={{ textAlign: "center", padding: "0.5rem 0" }}>
        <p style={{ fontWeight: 700, color: "#fff" }}>You&apos;re signed up!</p>
        <p style={{ color: "#ede9fe", fontSize: "0.88rem", marginTop: "0.25rem" }}>
          We&apos;ll send occasional app updates and announcements.
        </p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "0.55rem", maxWidth: "22rem", margin: "0 auto" }}>
      ...
      >
        {status === "loading" ? "Signing up..." : "Stay Updated"}
      </button>
      {status === "error" && (
